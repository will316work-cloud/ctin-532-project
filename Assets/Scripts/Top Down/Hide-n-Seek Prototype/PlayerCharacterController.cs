...
public class PlayerCharacterController : MonoBehaviour
{
    ...
    public UnityEvent OnJumpInputIn;
    public UnityEvent OnJumpInputOut;
    private bool m_JumpOn;
    ...
    void Update()
    {
        if (m_InputHandler.GetJumpInputDown())
        {
            if (m_JumpOn)
            {
                OnJumpInputOut?.Invoke();
                m_JumpOn = false;
            }
            else
            {
                OnJumpInputIn?.Invoke();
                m_JumpOn = true;
            }
        }
    ...
    {
        float r = m_Controller.radius * 1f; // Slightly smaller to avoid false positives
        Vector3 top = GetCapsuleTopHemisphere(m_Controller.height);
        Vector3 bottom = GetCapsuleBottomHemisphere();

        Gizmos.color = Color.red;
        // Draw where the cast starts
        Gizmos.DrawWireSphere(bottom, r);
        Gizmos.DrawWireSphere(top, r);

        // Draw where the cast ends
        Vector3 offset = Vector3.down * .07f;
        Gizmos.color = Color.green;
        Gizmos.DrawWireSphere(bottom + offset, r);
        Gizmos.DrawWireSphere(top + offset, r);
    }
    */
    ...
    void GroundCheck()
    {
        ...
        Debug.Log(chosenGroundCheckDistance);
